...
export const LLMTransformer = z.any().transform((data, ctx) => {
  ...
  if (Gemini31Pro.shape.jbai.value === data.jbai && 'capabilities' in data) {
    return safeParseOrAddIssues(Gemini31Pro, data, ctx)
  }
  if (Grok41Fast.shape.jbai.value === data.jbai && 'capabilities' in data) {
    return safeParseOrAddIssues(Grok41Fast, data, ctx)
  }
  ...
  if (AnthropicSonnet46.shape.jbai.value === data.jbai && 'capabilities' in data) {
    return safeParseOrAddIssues(AnthropicSonnet46, data, ctx)
  }
