...
const ProjectMetricsChart: Component = () => {
  return (
...
const ProjectMetricsChartOptions: Component = () => {
  return (
...
export const homeRouteHandler = async (req: AppRequest, res: AppResponse) => {
  ...
  try {
    ...
    const HtmlPage: Component<{ cookies: Record<string, any> }> = async ({ cookies, children }) => {
    ...
    const AppHeader: Component<{ actions: Children }> = ({ actions }) => {
      return (
    ...
    const AppBody: Component = ({ children }) => (
    ...
    const AppHead: Component<{ title: string }> = ({ title, children }) => (
      <head>
        <meta charset="UTF-8"/>
    ...
    const Conditional: Component<{ condition: boolean }> = ({ condition, children }) => {
      return condition ? children : ''
    }

    const IdeSelection: Component<{ ides: { name: string, src: string }[] }> = ({ ides }) => {
      return (
    ...
    const ProjectTable: Component<{ projects: Project[] }> = ({ projects }) => {
      return (
